"""
Verification script for SISAP 2026 Similarity Approximation (Optimized Version).

This script compares the ground-truth inner products of float16 embeddings 
against the EvpBits (sparsified ternary) similarity scores using the optimized implementation.
It helps to verify that the bitwise representation correctly approximates the 
original cosine similarity / inner product.

Usage:
    uv run python tests/test_similarity_optimized.py
"""
import time
import h5py
import numpy as np
...
from sisap.evp_optimized import EvpBits, evp_similarity, get_max_similarity
